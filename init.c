...
int ixgbe_probe(const struct hw *hw,volatile u8 *trace) {
    ...
    if (unlikely((err & IXGBE_CTRL_RST) | (err & IXGBE_CTRL_LRST) ))
        return -1;
    u8 i;
    for (i = 0; i < 50; i++) {
        const u32 eeprom = ixgbe_read_reg(hw,IXGBE_EEC);
        if (eeprom & IXGBE_EEC_ARD)
            goto eeprom_ok;
        usleep(1000);
    }
    if (unlikely((i == 50)))
    ...
    for (i = 0; i < 50; i++) {
        ...
        if (dmaidone & IXGBE_RDRXCTL_DMAIDONE)
            goto dmaiok;
        usleep(1000);
    }
    if (unlikely((i == 50)))
    ...
    const u32 blink = IXGBE_LED_CONF(0, 0x8E) |
              IXGBE_LED_CONF(1, 0x8E) |
              IXGBE_LED_CONF(2, 0x8E) |
              IXGBE_LED_CONF(3, 0x8E);
    ...
    err = ixgbe_read_reg(hw, IXGBE_LEDCTL);
    if (unlikely((err != ledctl)))
        return -1;
    /*
     * LEDCTL part is temporary, added for proving the work.
     * findings are;
     * Since we are managing 1 port, the second port's LED's didn't turn on green light.
     * The LEDs are not blinked since they're blinking with traffic, and we don't have one.
     * This means all the previous configurations are successful.
    */
    if (likely(trace)) {
        (*trace)++;
    }; return 0;
